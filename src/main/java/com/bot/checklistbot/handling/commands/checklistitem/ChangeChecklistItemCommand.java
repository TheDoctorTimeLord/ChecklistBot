...
/**
 * Команда изменения состояния пункта списка
 */
@Command
public class ChangeChecklistItemCommand implements BotCommand {
    ...
    @Autowired
    public ChangeChecklistItemCommand(ChecklistItemService checklistItemService) {
    ...
    public void initialize(UserData userData, String metadata) throws CommandInitializeException {
        ...
        if (item == null || item.getChecklist().getOwner().getId() != userData.getId()) {
    ...
    public String getCode() {
        return Commands.CHANGE_CHECKLIST_ITEM_COMMAND_CODE;
    }
    ...
    public BotResponse execute() {
        ...
        return new BotResponse(userData.getId(), Collections.singletonList(LocalizedText.checklistItemChanged(item)));
    }
}
